...
void DATAMCReader::Loop(bool isMC, TString Prod, TString sample, bool Signal, int  Year, bool isPostAPV, float MeanGenW, int Channel ,bool DoubleMuon, TString thesystlist)
{
  ...
  TString thesample  = sample;
   ...
   //   if(i==0) firstinit = true;
    TFile * theoutputfile = new TFile( ("../../"+Prod+"/DATAMC_TEST"+samplename+".root").Data() , "RECREATE");
...
if (thesample.Contains("smu200"))
  ...
  if (!isMC)//<=> Data
   ...
   // if (nentries > 10000000){nentries = 10000000; norm = 10000000.;}
      ...
      if (nentries > 100){nentries = 100; norm = 100;}
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (abs(minitree_lepton_leadingeta2->at(0))>1.5  && MuMu==0) 
         {
               // std::cout << "minitree_lepton_leadingeta2 : " << std::endl;
               // std::cout << "minitree_lepton_leadingeta2 : " <<minitree_lepton_leadingeta2->at(0)<< std::endl;
            ...
            for (unsigned int iMuon = 0; iMuon < minitree_lepton_leadingpt->size(); iMuon++)
               {
                  ...
                  if (MuMu == 0 && isMC)
                     {// leading letpn is muon by default
                              ...
                              if(minitree_lepton_leadingpt->at(iMuon) <= 15. ) {   Mu_Reco_SF1=1.; Mu_ID_SF1 = 1.;  Mu_ISO_SF1=1.; Mu_ID_SF1err = 1.;  Mu_ISO_SF1err=1.; } //muon ID, reco,ISO, Y axis  pt range 15-200,  x axis eta 0-2.4
                              ...
                              if(minitree_lepton_leadingpt->at(iMuon) >= 120. ) { Mu_ID_SF1 = 1.; Mu_ISO_SF1=1.;Mu_ID_SF1err = 1.; Mu_ISO_SF1err=1.;}
   ...
   if (Signal)
      ...
      fillHisto("Tree_filter", "", samplename, minitree_Filter->at(0), NormFactor);// Filter                                                                                                                          
      fillHisto("Vertices", "NoSel", samplename,minitree_nPV->at(0), Pref_PU_gen_wt);// no sel                                                                                                                              

      if (minitree_Filter->at(0) ){                                                                                                                                                                     
            
            fillHisto("Vertices_filtercut", "", samplename,minitree_nPV->at(0), NormFactor);// no sel               
            fillHisto("njet","NoSel",  samplename,minitree_njet->at(0), NormFactor);
            fillHisto("njetNOmu","NoSel",  samplename,minitree_njetNOmu->at(0), NormFactor);
         }

      fillHisto("Tree_Mumu","nosel", samplename,  minitree_Mmumu->at(0), NormFactor);/// offline only                                                                                                              

      if (MuMu == 0)
         {
            for (unsigned int k = 0 ; k < minitree_muon_pt->size() ; k++)
               {
                  if (minitree_muon_pt->at(k)>25)
                     {
                        fillHisto("muon_pt", "NoSel", samplename, minitree_muon_pt->at(k), NormFactor_mu);
                        fillHisto("muon_eta", "NoSel", samplename, minitree_muon_eta->at(k), NormFactor_mu);
                     }

               }
            for (unsigned int k = 0 ; k < minitree_electron_pt->size() ; k++)
               {
                  if (minitree_electron_pt->at(k)>25)
                     {
                        fillHisto("electron_pt", "NoSel", samplename, minitree_electron_pt->at(k), NormFactor_ele);
                        fillHisto("electron_eta", "NoSel", samplename, minitree_electron_eta->at(k), NormFactor_ele);
                     }
            ...
            for(unsigned int iMuon = 0; iMuon <minitree_lepton_leadingpt->size(); iMuon++)//test    after trigger and lepton selection cut                                       
               {
                  if (minitree_lepton_leadingpt->at(iMuon)>25)
                     {
                        fillHisto("leading_muon_pt", "reco",  samplename, minitree_lepton_leadingpt->at(iMuon), NormFactor_mu);
                        fillHisto("leading_muon_eta", "reco",  samplename, minitree_lepton_leadingeta->at(iMuon), NormFactor_mu);
                        fillHisto("leading_muon_phi", "reco",  samplename, minitree_lepton_leadingphi->at(iMuon), NormFactor_mu);
                        fillHisto("leading_muon_dxy", "reco", samplename,minitree_lepton_leadingdxy->at(iMuon), NormFactor_mu );
                        fillHisto("leading_muon_dz", "reco", samplename,minitree_lepton_leadingdz->at(iMuon), NormFactor_mu);
                     }

                  if (minitree_lepton_leadingpt2->at(iMuon)>25)
                     {
                        fillHisto("leading_lepton_pt", "reco", samplename, minitree_lepton_leadingpt2->at(iMuon), NormFactor_ele);
                        fillHisto("leading_lepton_eta", "reco", samplename, minitree_lepton_leadingeta2->at(iMuon), NormFactor_ele);
                        fillHisto("leading_lepton_phi", "reco", samplename, minitree_lepton_leadingphi2->at(iMuon), NormFactor_ele);
                        fillHisto("leading_lepton_dxy", "reco", samplename,minitree_lepton_leadingdxy2->at(iMuon), NormFactor_ele);
                        fillHisto("leading_lepton_dz", "reco", samplename,minitree_lepton_leadingdz2->at(iMuon), NormFactor_ele);
                        fillHisto2D("leading_lepton_pt_eta", "reco", samplename, minitree_lepton_leadingpt2->at(iMuon), minitree_lepton_leadingeta2->at(iMuon), NormFactor_ele);
                     }
      ...
      else if (MuMu > 0)
         {
            ...
            for(unsigned int iMuon = 0; iMuon <minitree_lepton_leadingpt->size(); iMuon ++)//test    after trigger and lepton selection cut                                       
               ...
               for(unsigned int iel = 0; iel <minitree_lepton_leadingpt2->size(); iel ++)//test    after trigger and lepton selection cut                                      
               {
                  ...
                  fillHisto("leading_lepton_pt", "reco", samplename, minitree_lepton_leadingpt2->at(iel), NormFactor_mu2);
                  fillHisto("leading_lepton_eta", "reco", samplename, minitree_lepton_leadingeta2->at(iel),NormFactor_mu2);
                  fillHisto("leading_lepton_phi", "reco", samplename, minitree_lepton_leadingphi2->at(iel), NormFactor_mu2);
                  fillHisto("leading_lepton_dxy", "reco", samplename,minitree_lepton_leadingdxy2->at(iel), NormFactor_mu2);
                  fillHisto("leading_lepton_dz", "reco", samplename,minitree_lepton_leadingdz2->at(iel), NormFactor_mu2);
                  fillHisto2D("leading_lepton_pt_eta", "reco", samplename, minitree_lepton_leadingpt2->at(iel), minitree_lepton_leadingeta2->at(iel), NormFactor_mu2);
                  //}//ele impact para
               }
         }
      ...
      for(unsigned int ill = 0; ill <minitree_ll_pt->size(); ill ++)//test                                                                                                        
          {
            fillHisto("Dilepton_pt", "NoSel", samplename, minitree_ll_pt->at(ill), NormFactor);
            fillHisto("Dilepton_eta", "NoSel", samplename, minitree_ll_eta->at(ill), NormFactor);
            fillHisto("Dilepton_phi", "NoSel", samplename, minitree_ll_phi->at(ill), NormFactor);
            fillHisto("Dilepton_mass", "NoSel", samplename, minitree_ll_mass->at(ill), NormFactor);
         }
      ...
      if (  minitree_Filter->at(0) && minitree_njetNOmu->at(0)>= 1) // !!!!! pour EMu: tester minitree_lepton_leadingeta2->at(iel)<1.5 (barrel region)
         {
            //   std::cout<<" here 0  "<<std::endl;
            fillHisto("LeadingLeptons_dR","",               samplename,  minitree_lepton_lepton_dR->at(0),NormFactor);//filter *1
            fillHisto("LeadingLeptons_dPhi","",             samplename,  minitree_lepton_lepton_dPhi->at(0),NormFactor);//filter *1
            fillHisto("LeadingJets_dR","",                  samplename,  minitree_jet_jet_dR->at(0),NormFactor);//jetnoMu>1 *1
            fillHisto("LeadingJets_dPhi","",                samplename,  minitree_jet_jet_dPhi->at(0),NormFactor);//jetnoMu>1 * 1
            // std::cout<<" here 0A  "<<std::endl;
            fillHisto("LeadingLeptonJet_dRmax","",           samplename,  minitree_muon_jet_dRmax->at(0),NormFactor);//jetnoMu>1 *2
            fillHisto("LeadingLeptonJet_dRmin","",           samplename,  minitree_muon_jet_dRmin->at(0),NormFactor);//jetnoMu>1 *2
            fillHisto("HemiAxis_Mu_dR","",                   samplename,  minitree_HemiMu_dR->at(0),NormFactor);//jetnoMu>1 *2
            fillHisto("HemiAxis_OpMu_dR","",                 samplename,  minitree_HemiMuOp_dR->at(0),NormFactor);//jetnoMu>1 *2

            fillHisto("Nmu","",                 samplename, minitree_nmu->at(0),NormFactor);

            for (unsigned int i = 0 ; i < minitree_muon_pt->size(); i++)
               {
                  fillHisto("Muon_pt","",             samplename, minitree_muon_pt->at(0),NormFactor);
                  fillHisto("Muon_PFIsoLoose","",     samplename, minitree_muon_PFIsoLoose->at(0),NormFactor);
                  fillHisto("Muon_MiniIsoTight","",   samplename, minitree_muon_MiniIsoTight->at(0),NormFactor);
               }

            fillHisto("HT","",            samplename,  minitree_HT->at(0),NormFactor);//chnager pour la variable asosciée à chauqe h"misphère
            fillHisto("LT","",            samplename,  minitree_LT->at(0),NormFactor);// c''st ok pour LT
            fillHisto("nTrks","",         samplename,  minitree_nTracks->at(0),NormFactor);
            fillHisto("nLostTracks","",   samplename,  minitree_nLostTracks->at(0),NormFactor);

            // --- CMSSW Version of V0 Candidates
            for (unsigned int i = 0 ; i < minitree_K0_mass->size(); i++ )
               {
                  fillHisto("K0_mass","", samplename, minitree_K0_mass->at(i),NormFactor);
                  fillHisto("K0_pt","", samplename, minitree_K0_pt->at(i),NormFactor);
               }
            for (unsigned int i = 0 ; i < minitree_L0_mass->size(); i++ )
               {
                  fillHisto("L0_mass","", samplename,minitree_L0_mass->at(i),NormFactor);
                  fillHisto("L0_pt","", samplename, minitree_L0_pt->at(i),NormFactor);
               }
            ...
            for (unsigned int i = 0 ; i <minitree_V0_reco_source->size(); i++ )
               {
                     if (minitree_V0_reco_source->at(i) == 1) 
                        {
                           fillHisto("Reco_K0_mass","",samplename,minitree_V0_reco_mass->at(i),NormFactor);
                           fillHisto("Reco_K0_pt","", samplename, minitree_V0_reco_pt->at(i),NormFactor);
                        }
                     else if (minitree_V0_reco_source->at(i) == 2) 
                        {
                           fillHisto("Reco_L0_mass","", samplename,minitree_V0_reco_mass->at(i),NormFactor);
                           fillHisto("Reco_L0_pt","",samplename, minitree_V0_reco_pt->at(i),NormFactor);
                        }
            ...
            for (unsigned int k = 0 ; k < minitree_jet_pt->size(); k++)
               {
                  if ( minitree_jet_pt->at(k)>30)
                     {
                        fillHisto("hData_jet_pt","",            samplename, minitree_jet_pt->at(k),NormFactor);
                        fillHisto("hData_jet_eta","",           samplename, minitree_jet_eta->at(k),NormFactor);
                        fillHisto("hData_jet_btag_Deepjet","",  samplename, minitree_jet_btag_DeepJet->at(k),NormFactor);
                        fillHisto("hData_jet_HadronFlavour","", samplename, minitree_jet_HadronFlavour->at(k),NormFactor);
                     }

               }
            if (minitree_jet_leadingpt->at(0)> 30)
               {
                  fillHisto("leading_jet_pt","",            samplename,  minitree_jet_leadingpt->at(0),NormFactor);
                  fillHisto("leading_jet_eta","",           samplename,  minitree_jet_leadingeta->at(0),NormFactor);
               }
            if (minitree_jet_leadingpt2->at(0)> 30)
               {
                  fillHisto("subleading_jet_pt","",         samplename,  minitree_jet_leadingpt2->at(0),NormFactor);
                  fillHisto("subleading_jet_eta","",        samplename,  minitree_jet_leadingeta2->at(0),NormFactor);
               }
            // fillHisto("subleading_jet_pt","",         samplename,  minitree_jet_leadingpt2->at(0),NormFactor);
            // fillHisto("subleading_jet_eta","",        samplename,  minitree_jet_leadingeta2->at(0),NormFactor);
            ...
            //       // fillHisto("track_nTracks","TRK", samplename,   minitree_nTracks->at(0),NormFactor); 
            //       // fillHisto("track_nLostTracks","TRK", samplename, minitree_nLostTracks->at(0),NormFactor);
            //       fillHisto("track_ipc","TRK", samplename,  minitree_track_ipc->at(iTrk),NormFactor);
                  
            //       //fillHisto("track_charge","TRK", samplename, minitree_track_charge->at(iTrk),NormFactor);
            //       //fillHisto("track_isHighPurity","TRK", samplename, minitree_track_isHighPurity->at(iTrk),NormFactor);

            //       fillHisto("track_dxyError","TRK", samplename, minitree_track_dxyError->at(iTrk),NormFactor);

            //       fillHisto("track_nHitPixel","TRK", samplename, minitree_track_nHitPixel->at(iTrk),NormFactor);
            //       fillHisto("track_nHitTIB","TRK", samplename, minitree_track_nHitTIB->at(iTrk),NormFactor);
            //       fillHisto("track_nHitTOB","TRK", samplename, minitree_track_nHitTOB->at(iTrk),NormFactor);
            //       fillHisto("track_nHitTEC","TRK", samplename, minitree_track_nHitTEC->at(iTrk),NormFactor);
            //       fillHisto("track_nHitPXB","TRK", samplename, minitree_track_nHitPXB->at(iTrk),NormFactor);
            //       fillHisto("track_nHitPXF","TRK", samplename, minitree_track_nHitPXF->at(iTrk),NormFactor);
            //       fillHisto("track_isHitPixel","TRK", samplename, minitree_track_isHitPixel->at(iTrk),NormFactor);
                  
            //       fillHisto("track_nLayers","TRK", samplename, minitree_track_nLayers->at(iTrk),NormFactor);
            //       fillHisto("track_nLayersPixel","TRK", samplename, minitree_track_nLayersPixel->at(iTrk),NormFactor);
            //       fillHisto("track_region","TRK", samplename, minitree_track_region->at(iTrk),NormFactor);
            //       // fillHisto("track_btag","TRK", samplename, minitree_track_btag->at(iTrk),NormFactor);
            //       //fillHisto("track_energy","TRK", samplename, minitree_track_energy->at(iTrk),NormFactor);
            //       // fillHisto("track_Hemi","TRK", samplename, minitree_track_Hemi->at(iTrk),NormFactor);
                     
            //       // fillHisto("track_TRACK_SIZE","TRK", samplename,minitree_TRACK_SIZE->at(0),NormFactor);
                  

            //       fillHisto("track_dxy","TRK", samplename,minitree_track_dxy->at(iTrk),NormFactor);
            //       fillHisto("track_dz","TRK", samplename,minitree_track_dz->at(iTrk),NormFactor);
            //       fillHisto("track_pt","TRK", samplename,minitree_track_pt->at(iTrk),NormFactor);
            //       fillHisto("track_eta","TRK", samplename,minitree_track_eta->at(iTrk),NormFactor);
            //       fillHisto("track_NChi2","TRK", samplename,minitree_track_NChi2->at(iTrk),NormFactor);
            //       fillHisto("track_nhits","TRK", samplename,minitree_track_nHit->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk10","TRK", samplename,minitree_track_ntrk10->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk20","TRK", samplename,minitree_track_ntrk20->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk30","TRK", samplename,minitree_track_ntrk30->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk40","TRK", samplename,minitree_track_ntrk40->at(iTrk),NormFactor);
            //       fillHisto("track_drSig","TRK", samplename,minitree_track_drSig->at(iTrk),NormFactor);
            //       fillHisto("track_dzSig","TRK", samplename,minitree_track_dzSig->at(iTrk),NormFactor);
            //       fillHisto("track_iJet","TRK", samplename,minitree_track_iJet->at(iTrk),NormFactor);// tracks matched with jets dR< 0.4
            //       fillHisto("track_track_Hemi_dR","TRK", samplename,minitree_track_Hemi_dR->at(iTrk),NormFactor);// dR b/w tracks and jets
            //       fillHisto("track_track_Hemi_dRmax","TRK", samplename,minitree_track_Hemi_dRmax->at(iTrk),NormFactor);
            //       fillHisto("track_lost","TRK", samplename,minitree_track_lost->at(iTrk),NormFactor);
            //       fillHisto("track_MVAVal","TRK", samplename,minitree_track_MVAval->at(iTrk),NormFactor);

            //       fillHisto("track_Track_firstHit","TRK", samplename,minitree_track_firstHit->at(iTrk),NormFactor);
            //       fillHisto("track_Track_firstHit_x","TRK", samplename,minitree_track_firstHit_x->at(iTrk),NormFactor);
            //       fillHisto("track_Track_firstHit_y","TRK", samplename,minitree_track_firstHit_y->at(iTrk),NormFactor);
            //       fillHisto("track_Track_firstHit_z","TRK", samplename,minitree_track_firstHit_z->at(iTrk),NormFactor);
            //       fillHisto2D("track_Track_firstHit_X_Y",  "TRK", samplename,minitree_track_firstHit_x->at(iTrk),minitree_track_firstHit_y->at(iTrk),NormFactor);
         ...
         for (unsigned int i = 0 ; i < minitree_Hemi_pt->size(); i++)
            {
               ...
               if ( minitree_Hemi_pt->at(i) > 30.  )
                  {
                     fillHisto("Hemi_pt","",       samplename,  minitree_Hemi_pt->at(i),NormFactor);
                     fillHisto("Hemi_eta","",      samplename,  minitree_Hemi_eta->at(i),NormFactor);
                     fillHisto("Hemi_nJet","",     samplename,  minitree_Hemi_njet->at(i),NormFactor);
                     fillHisto("Hemi_nJetNoMu","",samplename,   minitree_Hemi_njet_nomu->at(i),NormFactor);
                                    fillHisto("HemiMu_pt","",    samplename,  minitree_HemiMu_pt->at(i),NormFactor);
               fillHisto("HemiMu_Mass","",  samplename,   minitree_HemiMu_mass->at(i),NormFactor);
               fillHisto("Hemi_nTrks","",   samplename,  minitree_Hemi_nTrks->at(i),NormFactor);
               fillHisto("Hemi_Mass","",    samplename,  minitree_Hemi_mass->at(i),NormFactor);
                  }
         ...
         for (unsigned int i = 0 ; i < minitree_SecInt_pt->size(); i++)
            {
               if (minitree_SecInt_selec->at(i))
                  {
                     fillHisto("SecInt_mass","Selec", samplename, minitree_SecInt_mass->at(i), NormFactor);
                     fillHisto("SecInt_drSig","Selec", samplename, minitree_SecInt_drSig->at(i), NormFactor);
                     fillHisto("SecInt_pt","Selec", samplename, minitree_SecInt_pt->at(i), NormFactor);
                     fillHisto("SecInt_dzSig","Selec", samplename, minitree_SecInt_dzSig->at(i), NormFactor);
                     fillHisto("SecInt_selec","Selec", samplename, minitree_SecInt_selec->at(i), NormFactor);
                     fillHisto("SecInt_layer","Selec", samplename, minitree_SecInt_layer->at(i), NormFactor);
                     fillHisto("SecInt_r","Selec", samplename, minitree_SecInt_r->at(i), NormFactor);
                     fillHisto("SecInt_z","Selec", samplename, minitree_SecInt_z->at(i), NormFactor);
                     if ( minitree_SecInt_layer->at(i) )
                        {
                           fillHisto("SecInt_mass","TrackerMatched", samplename, minitree_SecInt_mass->at(i), NormFactor);
                           fillHisto("SecInt_drSig","TrackerMatched", samplename, minitree_SecInt_drSig->at(i), NormFactor);
                           fillHisto("SecInt_pt","TrackerMatched", samplename, minitree_SecInt_pt->at(i), NormFactor);
                           fillHisto("SecInt_dzSig","TrackerMatched", samplename, minitree_SecInt_dzSig->at(i), NormFactor);
                           fillHisto("SecInt_selec","TrackerMatched", samplename, minitree_SecInt_selec->at(i), NormFactor);
                           fillHisto("SecInt_layer","TrackerMatched", samplename, minitree_SecInt_layer->at(i), NormFactor);
                           fillHisto("SecInt_r","TrackerMatched", samplename, minitree_SecInt_r->at(i), NormFactor);
                           fillHisto("SecInt_z","TrackerMatched", samplename, minitree_SecInt_z->at(i), NormFactor);
         ...
         for (unsigned int i = 0 ; i < minitree_Hemi_Vtx_Mass->size(); i++)
            {
               fillHisto("Vtx_NChi2","",            samplename,  minitree_Hemi_Vtx_NChi2->at(i),NormFactor);
               fillHisto("Vtx_nTrks","",           samplename,  minitree_Hemi_Vtx_nTrks->at(i),NormFactor);
               fillHisto("Vtx_Mass","",         samplename,  minitree_Hemi_Vtx_Mass->at(i),NormFactor);
               fillHisto("Vtx_Dist","",        samplename,   minitree_Hemi_Vtx_dist->at(i),NormFactor);

               fillHisto("Vtx_Step","",            samplename,  minitree_Hemi_Vtx_step->at(i),NormFactor);
                  fillHisto("Vtx_dR","",           samplename,  minitree_Hemi_Vtx_dR->at(i),NormFactor);
            }
         // std::cout<<" here 6  "<<std::endl;
         for (unsigned int i = 0 ; i < minitree_Hemi_SecVtx_Mass->size(); i++)
            {
               fillHisto("SecVtx_NChi2","",            samplename,  minitree_Hemi_SecVtx_NChi2->at(i),NormFactor);
               fillHisto("SecVtx_nTrks","",           samplename,  minitree_Hemi_SecVtx_nTrks->at(i),NormFactor);
               fillHisto("SecVtx_Mass","",         samplename,  minitree_Hemi_SecVtx_Mass->at(i),NormFactor);
               fillHisto("SecVtx_Dist","",        samplename,   minitree_Hemi_SecVtx_dist->at(i),NormFactor);

               fillHisto("SecVtx_Step","",         samplename,  minitree_Hemi_SecVtx_step->at(i),NormFactor);
               fillHisto("SecVtx_dR","",        samplename,minitree_Hemi_SecVtx_dR->at(i),NormFactor);
            }

         // std::cout<<" here 7  "<<std::endl;
         for (unsigned int i = 0 ; i < minitree_Hemi_Vtx_BDT_nTrks->size() ; i++)
            {
               fillHisto("FinalVtx_nTrks","",           samplename,  minitree_Hemi_Vtx_BDT_nTrks->at(i),NormFactor);
               fillHisto("FinalVtx_Step","",          samplename, minitree_Hemi_Vtx_BDT_step->at(i),NormFactor);
               fillHisto("FinalVtx_Mass","",         samplename,  minitree_Hemi_Vtx_BDT_HMass->at(i),NormFactor);
               fillHisto("FinalVtx_HMass","",        samplename,  minitree_Hemi_Vtx_BDT_HMass->at(i),NormFactor);
            }
