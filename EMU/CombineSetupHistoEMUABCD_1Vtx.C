...
void GatherHistograms() {
    ...
    TString nVtx = "1Vtx";//2Vtx or 2VtxAll
...
TString ProdData                = "DATA_MUMU_"+YEAR+"_19_08_2024";
    ...
    if (Channel == "EMU")
        {
            ...
            ProdData = "DATA_EMU_"+YEAR+"_31_10_2024";
        }
    ...
    // !! Name of the histograms to gather from the ABCDEFGHI regions
    TString htitleA = "hData_CRlowlowpt_"+nVtx+"_"+Variable;
    TString htitleB = "hData_CRlooselowlowpt_"+nVtx+"_"+Variable;

    TString htitleC = "hData_CRTight_"+nVtx+"_"+Variable;
    TString htitleD = "hData_CRLoose_"+nVtx+"_"+Variable;//


    // !!  Name of the output file (that will be an input to the datacards)
    std::vector<TString> REGIONS = {

        "LowLowPT_1Vtx",    //A
        "Loose_LowLowPT_1Vtx",   //B

        "Tight_1Vtx",    //C
        "Loose_1Vtx"   //D

        };
    std::vector<TString> Plots = {"VtxMass_1Vtx_","STW_1Vtx_","LT_1Vtx"};
    ...
    for (unsigned int m = 0 ; m < Plots.size() ; m++)
        {
            ...
            TString outputFileName = "Tight_1Vtx.root";
            ...
            for (unsigned int j = 0 ; j < 1 ; j++) // loop over the data samples
                {
                    ...
                    htitleB = shortname+"_hData_CRlooselowlowpt_"+nVtx+"_"+Variable+"_";
                    
                    htitleC = shortname+"_hData_CRTight_"+nVtx+"_"+Variable+"_";
                    htitleD = shortname+"_hData_CRLoose_"+nVtx+"_"+Variable+"_";
                    ...
                    // for (unsigned int m = 0 ; m < HistoNames.size() ; m++)
                        // {
                            ...
                            inputFile->cd();
                            TH1F* hist = (TH1F*)gROOT->FindObject(htitleC) ; //= (TH1F*)inputFile->Get(HistoNames[m]);
                            if (!hist) {
                                    std::cerr << "Error retrieving histogram: " << htitleC << " from file: " << file << std::endl;
                                    inputFile->Close();
                                    continue;
                                }
                            ...
                            histClone->SetName("Tight_1Vtx");//+SYST[j]
                            outputFile->cd();
            ...
            // !! Get the predictions for signal region

            // for (unsigned int m = 0 ; m < Plots.size() ; m++)
            //     {
                    TString file2 = GlobalPath+"outputroot/ABCD_DATAMC_EMu/Tight_1Vtx_"+Plots[m]+".root";
                    // Open each file
            ...
            TString Directory = "/opt/sbg/cms/ui2_data1/pvaucell/CMSSW_11_3_4/src/HiggsAnalysis/CombinedLimit/data/RPV_UDD_"+YEAR+"/ABCD_"+Channel+"_"+MU+"_"+Sign+"_"+etamax+"_"+nVtx+"_"+Plots[m]+"/";
            ...
            // // Put a tempalte of a shape datacard inside the directory

            // TString datacard = "datacard_"+Plots[m]+".txt";
