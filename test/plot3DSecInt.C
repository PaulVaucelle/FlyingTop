...
void plot(std::vector<TString> Sample , TString YEAR, TString PU )
{
 TFile *f1;
TString Prod = "SecInt";
TString Year = YEAR;
...
TString xtitle = "M_{#tilde{#chi}^{0}_{1}} [GeV]"; 
TString ytitle = "decay length [cm]";  
TString ztitle = "a.u";
TH1F* histograms[nMC];
...
for (int i = 0; i < nMC; i++) {
  fX[i]->cd();
  htitle0 = Sample[i]+"_hData_reco_3DSecInt_"; 
...
/////////
  ...
  histograms[0]->SetMinimum(0.00001);
