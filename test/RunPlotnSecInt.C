...
if (First) 
    {
        // --- ctau fixed ---//
            ...
            gROOT->LoadMacro("plotnSecInt_ctau.C");
                ...
                {   
                    ...
                    for (unsigned int i = 0; i < 1; ++i) { // loop on the years (relevant only for the data)
                            for (unsigned int k = 0; k < 2; ++k) { // loop on the selection (Selec, TrackerMatched) 
                                for (unsigned int l = 0; l < 7; ++l) { // loop on the PU (empty, _PU25, _PU30, _PU35, _PU40, _PU45, _PU50)
...
else //works
    {
        // -mass depednance --// ---------------------------
            ...
            gROOT->LoadMacro("plotnSecInt_mass.C");
