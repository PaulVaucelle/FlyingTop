...
for (int i = 0; i < nMC; i++) {
  fX[i]->cd();
  // TString HISTONAME = Sample[i]+"_hData_reco_nAveSecInt_"+Suffix;
  // std::cout<<"histoname : "<<HISTONAME<<std::endl;
  htitle0 = Sample[i]+"_hData_reco_nAveSecInt_"+Suffix; 
...
TString lumiText = lumi_13TeV+lumi_sqrtS;
  ...
  posY_ = 1-t - relPosY*(1-t-b);
      ...
      latex.SetTextAlign(11);
      latex.DrawLatex(posX_+0.15, posY_-0.01, extraText);
	    }

 c1->Update();
 c1->SaveAs( msmu+"_"+mneu+"_fixedmass_fctau_AVE_"+Suffix+".pdf");
}
