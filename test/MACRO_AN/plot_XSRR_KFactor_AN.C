...
void plot_XSRR_KFactor_AN() {
    ...
    std::vector<double> y3 = {8.15,3.47,1.68,0.89,0.50,0.30,0.18}; // NLO+NLL B.Fuks
    std::vector<double> y4 = {7.03,3.03,1.48,0.78,0.44,0.26,0.16}; // LO B.Fuks
    ...
    std::vector<double> y3_PDF_up_percent = {3.9,4.2,4.5,4.9,5.2,5.7,6.2};// NLO+NLL B.Fuks
    std::vector<double> y3_PDF_down_percent = {4.9,5.3,5.7,6.1,6.7,7.0,7.4};// NLO+NLL B.Fuks
    std::vector<double> y4_PDF_up_percent = {1.3,2.8,3.9,4.8,5.7,6.4,7.0};// LO B.Fuks : no pdf variation provided
    std::vector<double> y4_PDF_down_percent = {1.6,2.9,3.8,4.5,5.2,5.7,6.2};// LO B.Fuks: no pdf variation provided
...
Int_t ColorLightBlue = color10.GetNumber();
    ...
    gr_ratio34->GetYaxis()->SetTitle("#sigma_{#tilde{#mu_{R}}#tilde{#mu_{R}}}^{NLO+NLL}/#sigma_{#tilde{#mu_{R}}#tilde{#mu_{R}}}^{LO}");
    gr_ratio34->SetTitle("");
