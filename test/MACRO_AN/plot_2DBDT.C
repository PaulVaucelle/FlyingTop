...
void plot2D( std::vector<TString> fileName, std::vector<TString> NAME) {
...
Int_t ColorLightBlue = color10.GetNumber();
    ...
    for (size_t i = 0; i < fileName.size(); ++i) {
        ...
        if (!file || file->IsZombie()) {
            std::cerr << "Erreur : Impossible d'ouvrir le fichier " << fileName[i] << std::endl;
            return;
        }

        TH2* hist = dynamic_cast<TH2*>(file->Get(histName));
        if (!hist) {
            std::cerr << "Erreur : Histogramme '" << histName << "' non trouvé dans " << fileName[i] << std::endl;
            file->Close();
            return;
        }
    ...
    for (size_t i = 0; i < histograms.size(); ++i) { //size is actually 1 :D
        ...
        if (i == 0) {
            ...
            histograms[i]->Draw("L"); // Le premier histogramme
            ...
            histograms[i]->SetMarkerColor(colors[i]);
            histograms[i]->Scale(1./histograms[i]->Integral());
            histograms[i]->Draw("L SAME"); // Les suivants
    ...
    // Ajouter une légende
    TLegend* legend = new TLegend(0.2, 0.2, 0.6, 0.5);
    ...
    for (size_t i = 0; i < histograms.size(); ++i) {
        legend->AddEntry(histograms[i], NAME[i], "l");
    }
