...
//  gROOT->ProcessLine(".L TreeABCDReader.C+");
  ...
  int YEAR = 2024;
  TString Prod2018 = "MC_MUMU_2024_23_04_2025";
   ...
   for (int i = 0 ; i< 12 ; i++) 
    {
        ...
        {
              Prod2018 = "MC_MUMU_2024_23_04_2025";
              if (systlist.size() == 0) return;
              else if (systlist[j] == "JECUp")
                {
                  Prod2018 = "MC_MUMU_2024_23_04_2025_JECUp";
                }
              else if (systlist[j] == "JECDown")
                {
                  Prod2018 = "MC_MUMU_2024_23_04_2025_JECDown";
                }
              else if (systlist[j] == "JERUp")
                {
                  Prod2018 = "MC_MUMU_2024_23_04_2025_JERUp";
                }
              else if (systlist[j] == "JERDown")
                {
                  Prod2018 = "MC_MUMU_2024_23_04_2025_JERDown";
                }
              else
                {
                  Prod2018 = "MC_MUMU_2024_23_04_2025";
                }
            TString Path = "/opt/sbg/cms/ui2_data1/pvaucell/CMSSW_14_0_20/src/FlyingTop/FlyingTop/test/"+Prod2018+"/Mini"+BKGSet[i]+".root";
            c.Reset();
            c.Add(Path);
            TreeABCDReader* t = new TreeABCDReader(&c,Prod2018, BKGSet[i],systlist[j]);
            // W.push_back(t->MeanGenWeight(BKGSet[i], Prod));//
            double mean = WGT[i];//W[i];//;
            t->Loop(isMC, Prod2018, BKGSet[i],false, SameSign, Forward, YEAR, mean, DoubleMuon, mixing, Channel, isPostAPV,systlist[j],CorrectCorrelation);
            delete t;
        }
    }
