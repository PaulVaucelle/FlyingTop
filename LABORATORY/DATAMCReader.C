...
void DATAMCReader::Loop(bool isMC, TString Prod, TString sample, bool Signal, int  Year, bool isPostAPV, float MeanGenW, int Channel ,bool DoubleMuon, TString thesystlist)
{
  ...
  TString thesample  = sample;
   ...
   //   if(i==0) firstinit = true;
    TFile * theoutputfile = new TFile( ("./DATAMC_"+samplename+".root").Data() , "RECREATE");
   ...
   if (!Signal && isMC)
      {
         TString hNorma = "hEvents";
         TFile* f1_DY= new TFile("/opt/sbg/cms/ui2_data1/pvaucell/CMSSW_10_6_30_FLY/src/FlyingTop/FlyingTop/test/MC_EMU_03_02_2025/"+sample+".root");
         f1_DY->cd("");
  ...
  Long64_t nentries = fChain->GetEntriesFast();
   ...
   int MSmuon = 0;
   if (thesample.Contains("DYJetsToLL_M-10to50"))                     { XS = 22635;   }//ok : 15910 :https://doi.org/10.48550/arXiv.2402.08486
   if (thesample.Contains("DYJetsToLL_M-50"))                         { XS = 6225.4;      }//5379.0;  ok  }https://doi.org/10.48550/arXiv.2402.08486
   if (thesample.Contains("ST_tW_antitop_5f_NoFullyHadronicDecays")) { XS = 19.55;   }//21.6 ok
   if (thesample.Contains("ST_tW_top_5f_NoFullyHadronicDecays"))     { XS = 19.55;   }//21.6 ok
   ...
   if (thesample.Contains("TTTo2L2Nu"))                              { XS = 89.4;      }//ok
   ...
   if (thesample.Contains("ST_t-channel_antitop_5f_InclusiveDecays")) {XS = 80.95;}
   if (thesample.Contains("ST_t-channel_top_5f_InclusiveDecays")) {XS = 136.02;}
   //Signal
...
float allevents = 0;
   ...
   // if (jentry > 10000) break;
      allevents++;
      if ( jentry % 1000000 == 0 ) std::cout << "events : " << allevents << std::endl;
      ...
      if ( tree_Hemi_pt->size() != 2 ) continue;
      if ( abs(tree_Hemi_eta->at(0)) > 2.4 || abs(tree_Hemi_eta->at(1)) > 2.4 ) continue;
      if ( tree_Hemi_pt->at(0) < 20. || tree_Hemi_pt->at(1) < 20. ) continue;

      // if (abs(tree_lepton_leadingeta2->at(0))>1.5) continue;
      // !!
      // std::cout<<" here 0 "<<std::endl;
      // std::cout << "tree_lepton_leadingeta2.size() : " <<tree_lepton_leadingeta2->size()<< std::endl;
      if (abs(tree_lepton_leadingeta2->at(0))>1.5  && MuMu==0) 
         {
               // std::cout << "tree_lepton_leadingeta2 : " << std::endl;
               // std::cout << "tree_lepton_leadingeta2 : " <<tree_lepton_leadingeta2->at(0)<< std::endl;
            SFele_eta = hEle_SF->GetBinContent(hEle_SF->GetXaxis()->FindBin(tree_lepton_leadingeta2->at(0)));
            SFele_eta_err = hEle_SF->GetBinError(hEle_SF->GetXaxis()->FindBin(tree_lepton_leadingeta2->at(0)));
            // std::cout<<"SFele_eta : "<<SFele_eta<<std::endl;
            // SFele_pt_eta = hEle_2DSF->GetBinContent(hEle_2DSF->GetXaxis()->FindBin(tree_lepton_leadingpt2->at(0)),hEle_2DSF->GetYaxis()->FindBin(tree_lepton_leadingeta2->at(0)));
         }
      // !! 
      if (tree_Filter)
         {
               // std::cout<<"here1 : "<<std::endl;
            for (unsigned int iMuon = 0; iMuon < tree_lepton_leadingpt->size(); iMuon++)
               {
                  if (tree_lepton_leadingpt->at(0) == 0   )
                     {
                        Mu_Reco_SF1 = 1;
                        Mu_ID_SF1 = 1;	    
                        Mu_ISO_SF1 = 1 ;
                  ...
                  if (MuMu == 0 && isMC)
                     {// leading letpn is muon by default
                              ...
                              mu1_pt = tree_lepton_leadingpt->at(iMuon);
                              Mu_Reco_SF1= fh2DL1_Reco_SF1->GetBinContent(fh2DL1_Reco_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_Reco_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));
                              Mu_ID_SF1 = fh2DL1_ID_SF1->GetBinContent(fh2DL1_ID_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ID_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));	    
                              Mu_ID_SF1err = fh2DL1_ID_SF1err->GetBinContent(fh2DL1_ID_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ID_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));
                              Mu_ISO_SF1 = fh2DL1_ISO_SF1->GetBinContent(fh2DL1_ISO_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ISO_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));//fh2DL1_ISO_SF1->GetBinContent(fh2DL1_ISO_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ISO_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));
                              Mu_ISO_SF1err = fh2DL1_ISO_SF1err->GetBinContent(fh2DL1_ISO_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ISO_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));//fh2DL1_ISO_SF1->GetBinContent(fh2DL1_ISO_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ISO_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));
                              if(tree_lepton_leadingpt->at(iMuon) <= 15. ) {   Mu_Reco_SF1=1.; Mu_ID_SF1 = 1.;  Mu_ISO_SF1=1.; Mu_ID_SF1err = 1.;  Mu_ISO_SF1err=1.; } //muon ID, reco,ISO, Y axis  pt range 15-200,  x axis eta 0-2.4
                              // One has to perform the difference Mu_ID_SF1 and Mu_ID_SF1err to get the error so you hav eto set the default value of Mu_ID_SF1err to 1, but this is only true for MuMu == 0
                              if(tree_lepton_leadingpt->at(iMuon) > 40  ) { Mu_Reco_SF1 = 1. ;} 
                              if(tree_lepton_leadingpt->at(iMuon) >= 120. ) { Mu_ID_SF1 = 1.; Mu_ISO_SF1=1.;Mu_ID_SF1err = 1.; Mu_ISO_SF1err=1.;}
                  ...
                  else if (MuMu > 0 && isMC) // $$$$$
                     {
                        ...
                        mu1_pt = tree_lepton_leadingpt->at(iMuon);
                        mu1_eta = tree_lepton_leadingeta->at(iMuon);
                        mu2_pt = tree_lepton_leadingpt2->at(iMuon);

                        Mu_Reco_SF1= fh2DL1_Reco_SF1->GetBinContent(fh2DL1_Reco_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_Reco_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));
                        Mu_ID_SF1 = fh2DL1_ID_SF1->GetBinContent(fh2DL1_ID_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ID_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));	    
                        Mu_ISO_SF1 = fh2DL1_ISO_SF1->GetBinContent(fh2DL1_ISO_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ISO_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));
                        Mu_ID_SF1err = fh2DL1_ID_SF1err->GetBinContent(fh2DL1_ID_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ID_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));	    
                        Mu_ISO_SF1err = fh2DL1_ISO_SF1err->GetBinContent(fh2DL1_ISO_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta->at(iMuon))),fh2DL1_ISO_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt->at(iMuon)));

                        
                        Mu_Reco_SF2= fh2DL1_Reco_SF1->GetBinContent(fh2DL1_Reco_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iMuon))),fh2DL1_Reco_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iMuon)));
                        Mu_ID_SF2 = fh2DL1_ID_SF1->GetBinContent(fh2DL1_ID_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iMuon))),fh2DL1_ID_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iMuon)));	    
                        Mu_ISO_SF2 = fh2DL1_ISO_SF1->GetBinContent(fh2DL1_ISO_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iMuon))),fh2DL1_ISO_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iMuon)));
                        Mu_ID_SF2err = fh2DL1_ID_SF1err->GetBinContent(fh2DL1_ID_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iMuon))),fh2DL1_ID_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iMuon)));	    
                        Mu_ISO_SF2err = fh2DL1_ISO_SF1err->GetBinContent(fh2DL1_ISO_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iMuon))),fh2DL1_ISO_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iMuon)));


                        if(tree_lepton_leadingpt->at(iMuon) <= 15. ) {   Mu_Reco_SF1=1.; Mu_ID_SF1 = 1.;  Mu_ISO_SF1=1.;  Mu_ID_SF1err = 0.;  Mu_ISO_SF1err=0.;  } //muon ID, reco,ISO, Y axis  pt range 15-200,  x axis eta 0-2.4
                        if(tree_lepton_leadingpt->at(iMuon) > 40  ) { Mu_Reco_SF1 = 1. ;} 
                        if(tree_lepton_leadingpt->at(iMuon) >= 120. ) { Mu_ID_SF1 = 1.; Mu_ISO_SF1=1.;Mu_ID_SF1err = 0.; Mu_ISO_SF1err=0.;}

                        
                        if(tree_lepton_leadingpt2->at(iMuon) <= 15. ) {   Mu_Reco_SF2=1.; Mu_ID_SF2 = 1.;  Mu_ISO_SF2=1.; Mu_ID_SF2err = 0.;  Mu_ISO_SF2err=0.; } //muon ID, reco,ISO, Y axis  pt range 15-200,  x axis eta 0-2.4
                        if(tree_lepton_leadingpt2->at(iMuon) > 40  ) { Mu_Reco_SF2 = 1. ;} 
                        if(tree_lepton_leadingpt2->at(iMuon) >= 120. ) { Mu_ID_SF2 = 1.; Mu_ISO_SF2=1.;Mu_ID_SF2err = 0.; Mu_ISO_SF2err=0.;}   
            ...
            if (MuMu == 0 && isMC)
               {
                     for(unsigned int iel = 0; iel <tree_lepton_leadingpt2->size(); iel ++)//test    after trigger and lepton selection cut                                    
                        {
                           //  std::cout<<"here4 : "<<std::endl;
                          if (tree_lepton_leadingpt2->at(0) == 0  )
                           {
                              Ele_Reco_SF1 = 1;
                              Ele_ID_SF1 = 1;	
                              Ele_ID_SF1err = 0;   
                           ...
                           ele1_pt = tree_lepton_leadingpt2->at(iel);
                           ele1_eta = fabs(tree_lepton_leadingeta2->at(iel));
                           if ( ele1_pt < 20)
                              {  
                                 Ele_Reco_SF1 = fh2DL2_ISO_SF1->GetBinContent(fh2DL2_ISO_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iel))),fh2DL2_ISO_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iel)));
                                 Ele_Reco_SF1err = fh2DL2_ISO_SF1err->GetBinContent(fh2DL2_ISO_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iel))),fh2DL2_ISO_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iel)));
                              }
                           else
                              {
                                 Ele_Reco_SF1 = fh2DL2_Reco_SF1->GetBinContent(fh2DL2_Reco_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iel))),fh2DL2_Reco_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iel)));
                                 Ele_Reco_SF1err = fh2DL2_Reco_SF1err->GetBinContent(fh2DL2_Reco_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iel))),fh2DL2_Reco_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iel)));
                              }
                           Ele_ID_SF1 = fh2DL2_ID_SF1->GetBinContent(fh2DL2_ID_SF1->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iel))),fh2DL2_ID_SF1->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iel)));
                           Ele_ID_SF1err = fh2DL2_ID_SF1err->GetBinContent(fh2DL2_ID_SF1err->GetXaxis()->FindBin(fabs(tree_lepton_leadingeta2->at(iel))),fh2DL2_ID_SF1err->GetYaxis()->FindBin(tree_lepton_leadingpt2->at(iel)));

                           if(tree_lepton_leadingpt2->at(iel) <= 10. ) {  Ele_ID_SF1=1.;Ele_ID_SF1err=0.;Ele_Reco_SF1err = 0; }// ID, 10-500, eta -2.5 to 2.5, x axis = eta, y axis=pt   
                           if(tree_lepton_leadingpt2->at(iel) <= 10. ) {  Ele_Reco_SF1 = 1.;  }// Reco , 10-500, eta -2.5 to 2.5, x axis = eta, y axis=pt
                           if(tree_lepton_leadingpt2->at(iel) >= 500. ) { Ele_Reco_SF1 = 1.; Ele_ID_SF1err=0.; Ele_Reco_SF1err = 0;}
         ...
         if (isMC && MuMu == 0) 
            {   
               ...
               if (ele1_pt < 15. || mu1_pt < 15. ||  ele1_pt > 200 || mu1_pt > 200){triggerSF=1.;triggerSFerr=0.;skippy = true;} // x and y axis 15 to 200
               if (!skippy)
                  {
      ...
      // --------- Event SF ------//
         // -- PDF ----//
   float GenWeight = tree_only_gen_wt/MeanGenW;
   ...
   if (!isMC)//<=> Data
      ...
      //  std::cout<<"here7 : "<<std::endl;
         PUWeight = PUweight;
         PUWeightUp = PUweight;
         PUWeightDown = PUweight_Down;
         PrefWeight = Prefweight;
         PrefWeightUp = Prefweight_Up;
         PrefWeightDown = Prefweight_Down;
         ...
         if (systlist.Contains("LumiUp"))
            ...
            {  
               ...
               for (unsigned int i = 0; i < tree_LHE_Weights->size(); i++)
                  {
                     if (i >=10 || i == 0) {PDFWeights.push_back(tree_LHE_Weights->at(i));}
                     else  {ScaleWeights.push_back(tree_LHE_Weights->at(i));}//if (i < 10 && i >=1)
                  }
         ...
         else if (systlist.Contains("PDFDown"))
            {
               for (unsigned int i = 0; i < tree_LHE_Weights->size(); i++)
                  {
                     if (i >=10 || i == 0) {PDFWeights.push_back(tree_LHE_Weights->at(i));}
                     else if (i < 10 && i >=1) {ScaleWeights.push_back(tree_LHE_Weights->at(i));}
                  }
   ...
   if (Signal)
      {
            ...
            // std::cout<<"NormFactorSYST="<<NormFactorSYST<<" GenWeight="<<GenWeight<<" Prefweight="<<Prefweight<<" PUweight="<<PUweight<<" top_pt_wt="<<top_pt_wt<<" Mu_SF="<<Mu_SF<<" Ele_SF="<<Ele_SF<<" EMu_trig_SF1="<<EMu_trig_SF1<<std::endl;
            // std::cout<<" XS = "<<XS<<" Lumi = "<<Lumi<<" nentries = "<<nentries<<std::endl;
            // std::cout<<"MeanGenW : "<<MeanGenW<<"and genweight : "<<tree_only_gen_wt->at(0)<<std::endl;
            //  std::cout<<"SFele_eta = "<<SFele_eta<<" and the eta of leadingletpno 2 : "<< tree_lepton_leadingeta2->at(0)<<std::endl;
      ...
      fillHisto("Tree_filter", "", samplename, tree_Filter, NormFactor);// Filter                                                                                                                          
      fillHisto("Vertices", "NoSel", samplename,tree_nPV, Pref_PU_gen_wt);// no sel                                                                                                                              

      if (tree_Filter){                                                                                                                                                                     
            
            fillHisto("Vertices_filtercut", "", samplename,tree_nPV, NormFactor);// no sel               
            fillHisto("njet","NoSel",  samplename,tree_njet, NormFactor);
            fillHisto("njetNOmu","NoSel",  samplename,tree_njetNOmu, NormFactor);
         }

      fillHisto("Tree_Mumu","nosel", samplename,  tree_Mmumu, NormFactor);/// offline only                                                                                                              

      if (MuMu == 0)
         {
            for (unsigned int k = 0 ; k < tree_muon_pt->size() ; k++)
               {
                  fillHisto("muon_pt", "NoSel", samplename, tree_muon_pt->at(k), NormFactor_mu);
                  fillHisto("muon_eta", "NoSel", samplename, tree_muon_eta->at(k), NormFactor_mu);
               }
            for (unsigned int k = 0 ; k < tree_electron_pt->size() ; k++)
               {
                  fillHisto("electron_pt", "NoSel", samplename, tree_electron_pt->at(k), NormFactor_ele);
                  fillHisto("electron_eta", "NoSel", samplename, tree_electron_eta->at(k), NormFactor_ele);
               }


            for(unsigned int iMuon = 0; iMuon <tree_lepton_leadingpt->size(); iMuon++)//test    after trigger and lepton selection cut                                       
               {

                  fillHisto("leading_muon_pt", "reco",  samplename, tree_lepton_leadingpt->at(iMuon), NormFactor_mu);
                  fillHisto("leading_muon_eta", "reco",  samplename, tree_lepton_leadingeta->at(iMuon), NormFactor_mu);
                  fillHisto("leading_muon_phi", "reco",  samplename, tree_lepton_leadingphi->at(iMuon), NormFactor_mu);
                  fillHisto("leading_muon_dxy", "reco", samplename,tree_lepton_leadingdxy->at(iMuon), NormFactor_mu );
                  fillHisto("leading_muon_dz", "reco", samplename,tree_lepton_leadingdz->at(iMuon), NormFactor_mu);

                  fillHisto("leading_lepton_pt", "reco", samplename, tree_lepton_leadingpt2->at(iMuon), NormFactor_ele);
                  fillHisto("leading_lepton_eta", "reco", samplename, tree_lepton_leadingeta2->at(iMuon), NormFactor_ele);
                  fillHisto("leading_lepton_phi", "reco", samplename, tree_lepton_leadingphi2->at(iMuon), NormFactor_ele);
                  fillHisto("leading_lepton_dxy", "reco", samplename,tree_lepton_leadingdxy2->at(iMuon), NormFactor_ele);
                  fillHisto("leading_lepton_dz", "reco", samplename,tree_lepton_leadingdz2->at(iMuon), NormFactor_ele);
                  fillHisto2D("leading_lepton_pt_eta", "reco", samplename, tree_lepton_leadingpt2->at(iMuon), tree_lepton_leadingeta2->at(iMuon), NormFactor_ele);
               }
      ...
      else if (MuMu > 0)
         {
            ...
            for(unsigned int iMuon = 0; iMuon <tree_lepton_leadingpt->size(); iMuon ++)//test    after trigger and lepton selection cut                                       
               {
                  //if(tree_muon_dxy->at(iMuon)< 0.1  && tree_muon_dz->at(iMuon) < 0.2 ){ 
                  fillHisto("leading_muon_pt", "reco",  samplename, tree_lepton_leadingpt->at(iMuon), NormFactor_mu);
                  fillHisto("leading_muon_eta", "reco",  samplename, tree_lepton_leadingeta->at(iMuon), NormFactor_mu);
                  fillHisto("leading_muon_phi", "reco",  samplename, tree_lepton_leadingphi->at(iMuon), NormFactor_mu);
                  fillHisto("leading_muon_dxy", "reco", samplename,tree_lepton_leadingdxy->at(iMuon), NormFactor_mu );
               ...
               for(unsigned int iel = 0; iel <tree_lepton_leadingpt2->size(); iel ++)//test    after trigger and lepton selection cut                                      
               {
                  //if (  ( fabs(tree_reco_electron_leadingeta2->at(iel)) <= 1.479 && abs(tree_electron_dxy->at(iel)) < 0.05 && abs(tree_electron_dz->at(iel) ) < 0.10 ) || ( fabs(tree_reco_electron_leadingeta2->at(iel)) > 1.556 && abs(tree_electron_dxy->at(iel)) < 0.10 && abs(tree_electron_dz->at(iel)) < 0.20 )){

                  fillHisto("leading_lepton_pt", "reco", samplename, tree_lepton_leadingpt2->at(iel), NormFactor_mu2);
                  fillHisto("leading_lepton_eta", "reco", samplename, tree_lepton_leadingeta2->at(iel),NormFactor_mu2);
                  fillHisto("leading_lepton_phi", "reco", samplename, tree_lepton_leadingphi2->at(iel), NormFactor_mu2);
                  fillHisto("leading_lepton_dxy", "reco", samplename,tree_lepton_leadingdxy2->at(iel), NormFactor_mu2);
                  fillHisto("leading_lepton_dz", "reco", samplename,tree_lepton_leadingdz2->at(iel), NormFactor_mu2);
                  fillHisto2D("leading_lepton_pt_eta", "reco", samplename, tree_lepton_leadingpt2->at(iel), tree_lepton_leadingeta2->at(iel), NormFactor_mu2);
                  //}//ele impact para
               }
         }
      ...
      //     {
      //       fillHisto("Dilepton_pt", "NoSel", samplename, tree_ll_pt->at(ill), NormFactor);
      //       fillHisto("Dilepton_eta", "NoSel", samplename, tree_ll_eta->at(ill), NormFactor);
      //       fillHisto("Dilepton_phi", "NoSel", samplename, tree_ll_phi->at(ill), NormFactor);
      //       fillHisto("Dilepton_mass", "NoSel", samplename, tree_ll_mass->at(ill), NormFactor);
      //    }
      ...
      if (MuMu == 2 || MuMu == 1)
         {
            if (DoubleMuon && tree_trigger_doublelepton  ) //&& !tree_trigger_singlelepton
               {
                  FilterSample = 2;
               }
            else if  (!DoubleMuon && tree_trigger_singlelepton &&  !tree_trigger_doublelepton ) 
               {
                  FilterSample = 1;
               }
         }
      ...
      if (  tree_Filter && tree_njetNOmu >= 1) // !!!!! pour EMu: tester tree_lepton_leadingeta2->at(iel)<1.5 (barrel region)
         {
            //   std::cout<<" here 0  "<<std::endl;
            fillHisto("LeadingLeptons_dR","",               samplename,  tree_lepton_lepton_dR->at(0),NormFactor);//filter *1
            fillHisto("LeadingLeptons_dPhi","",             samplename,  tree_lepton_lepton_dPhi->at(0),NormFactor);//filter *1
            fillHisto("LeadingJets_dR","",                  samplename,  tree_jet_jet_dR->at(0),NormFactor);//jetnoMu>1 *1
            fillHisto("LeadingJets_dPhi","",                samplename,  tree_jet_jet_dPhi->at(0),NormFactor);//jetnoMu>1 * 1
            // std::cout<<" here 0A  "<<std::endl;
            fillHisto("LeadingLeptonJet_dRmax","",           samplename,  tree_muon_jet_dRmax->at(0),NormFactor);//jetnoMu>1 *2
            fillHisto("LeadingLeptonJet_dRmin","",           samplename,  tree_muon_jet_dRmin->at(0),NormFactor);//jetnoMu>1 *2
            fillHisto("HemiAxis_Mu_dR","",                   samplename,  tree_HemiMu_dR->at(0),NormFactor);//jetnoMu>1 *2
            fillHisto("HemiAxis_OpMu_dR","",                 samplename,  tree_HemiMuOp_dR->at(0),NormFactor);//jetnoMu>1 *2

            fillHisto("Nmu","",                 samplename, tree_nmu,NormFactor);

            for (unsigned int i = 0 ; i < tree_muon_pt->size(); i++)
               {
                  fillHisto("Muon_pt","",             samplename, tree_muon_pt->at(0),NormFactor);
                  fillHisto("Muon_PFIsoLoose","",     samplename, tree_muon_PFIsoLoose->at(0),NormFactor);
                  fillHisto("Muon_MiniIsoTight","",   samplename,1,NormFactor);
               }

            fillHisto("HT","",            samplename,  tree_HT,NormFactor);//chnager pour la variable asosciée à chauqe h"misphère
            fillHisto("LT","",            samplename,  tree_LT,NormFactor);// c''st ok pour LT
            fillHisto("nTrks","",         samplename,  tree_nTracks,NormFactor);
            fillHisto("nLostTracks","",   samplename,  tree_nLostTracks,NormFactor);

            // --- CMSSW Version of V0 Candidates
            for (unsigned int i = 0 ; i < tree_K0_mass->size(); i++ )
               {
                  fillHisto("K0_mass","", samplename, tree_K0_mass->at(i),NormFactor);
                  fillHisto("K0_pt","", samplename, tree_K0_pt->at(i),NormFactor);
               }
            for (unsigned int i = 0 ; i < tree_L0_mass->size(); i++ )
               {
                  fillHisto("L0_mass","", samplename,tree_L0_mass->at(i),NormFactor);
                  fillHisto("L0_pt","", samplename, tree_L0_pt->at(i),NormFactor);
               }

            // -- Our own collection of V0 Candidates
            for (unsigned int i = 0 ; i <tree_V0_reco_source->size(); i++ )
               {
                     if (tree_V0_reco_source->at(i) == 1) 
                        {
                           fillHisto("Reco_K0_mass","",samplename,tree_V0_reco_mass->at(i),NormFactor);
                           fillHisto("Reco_K0_pt","", samplename, tree_V0_reco_pt->at(i),NormFactor);
                        }
                     else if (tree_V0_reco_source->at(i) == 2) 
                        {
                           fillHisto("Reco_L0_mass","", samplename,tree_V0_reco_mass->at(i),NormFactor);
                           fillHisto("Reco_L0_pt","",samplename, tree_V0_reco_pt->at(i),NormFactor);
                        }
            ...
            for (unsigned int k = 0 ; k < tree_jet_pt->size(); k++)
               {
                  fillHisto("hData_jet_pt","",            samplename, tree_jet_pt->at(k),NormFactor);
                  fillHisto("hData_jet_eta","",           samplename, tree_jet_eta->at(k),NormFactor);
                  fillHisto("hData_jet_btag_Deepjet","",  samplename, tree_jet_btag_DeepJet->at(k),NormFactor);
                  fillHisto("hData_jet_HadronFlavour","", samplename, tree_jet_HadronFlavour->at(k),NormFactor);
               }
            fillHisto("leading_jet_pt","",            samplename,  tree_jet_leadingpt->at(0),NormFactor);
            fillHisto("leading_jet_eta","",           samplename,  tree_jet_leadingeta->at(0),NormFactor);
            fillHisto("subleading_jet_pt","",         samplename,  tree_jet_leadingpt2->at(0),NormFactor);
            fillHisto("subleading_jet_eta","",        samplename,  tree_jet_leadingeta2->at(0),NormFactor);
            ...
            //       // fillHisto("track_nTracks","TRK", samplename,   tree_nTracks->at(0),NormFactor); 
            //       // fillHisto("track_nLostTracks","TRK", samplename, tree_nLostTracks->at(0),NormFactor);
            //       fillHisto("track_ipc","TRK", samplename,  tree_track_ipc->at(iTrk),NormFactor);
                  
            //       //fillHisto("track_charge","TRK", samplename, tree_track_charge->at(iTrk),NormFactor);
            //       //fillHisto("track_isHighPurity","TRK", samplename, tree_track_isHighPurity->at(iTrk),NormFactor);

            //       fillHisto("track_dxyError","TRK", samplename, tree_track_dxyError->at(iTrk),NormFactor);

            //       fillHisto("track_nHitPixel","TRK", samplename, tree_track_nHitPixel->at(iTrk),NormFactor);
            //       fillHisto("track_nHitTIB","TRK", samplename, tree_track_nHitTIB->at(iTrk),NormFactor);
            //       fillHisto("track_nHitTOB","TRK", samplename, tree_track_nHitTOB->at(iTrk),NormFactor);
            //       fillHisto("track_nHitTEC","TRK", samplename, tree_track_nHitTEC->at(iTrk),NormFactor);
            //       fillHisto("track_nHitPXB","TRK", samplename, tree_track_nHitPXB->at(iTrk),NormFactor);
            //       fillHisto("track_nHitPXF","TRK", samplename, tree_track_nHitPXF->at(iTrk),NormFactor);
            //       fillHisto("track_isHitPixel","TRK", samplename, tree_track_isHitPixel->at(iTrk),NormFactor);
                  
            //       fillHisto("track_nLayers","TRK", samplename, tree_track_nLayers->at(iTrk),NormFactor);
            //       fillHisto("track_nLayersPixel","TRK", samplename, tree_track_nLayersPixel->at(iTrk),NormFactor);
            //       fillHisto("track_region","TRK", samplename, tree_track_region->at(iTrk),NormFactor);
            //       // fillHisto("track_btag","TRK", samplename, tree_track_btag->at(iTrk),NormFactor);
            //       //fillHisto("track_energy","TRK", samplename, tree_track_energy->at(iTrk),NormFactor);
            //       // fillHisto("track_Hemi","TRK", samplename, tree_track_Hemi->at(iTrk),NormFactor);
                     
            //       // fillHisto("track_TRACK_SIZE","TRK", samplename,tree_TRACK_SIZE->at(0),NormFactor);
                  

            //       fillHisto("track_dxy","TRK", samplename,tree_track_dxy->at(iTrk),NormFactor);
            //       fillHisto("track_dz","TRK", samplename,tree_track_dz->at(iTrk),NormFactor);
            //       fillHisto("track_pt","TRK", samplename,tree_track_pt->at(iTrk),NormFactor);
            //       fillHisto("track_eta","TRK", samplename,tree_track_eta->at(iTrk),NormFactor);
            //       fillHisto("track_NChi2","TRK", samplename,tree_track_NChi2->at(iTrk),NormFactor);
            //       fillHisto("track_nhits","TRK", samplename,tree_track_nHit->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk10","TRK", samplename,tree_track_ntrk10->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk20","TRK", samplename,tree_track_ntrk20->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk30","TRK", samplename,tree_track_ntrk30->at(iTrk),NormFactor);
            //       fillHisto("track_ntrk40","TRK", samplename,tree_track_ntrk40->at(iTrk),NormFactor);
            //       fillHisto("track_drSig","TRK", samplename,tree_track_drSig->at(iTrk),NormFactor);
            //       fillHisto("track_dzSig","TRK", samplename,tree_track_dzSig->at(iTrk),NormFactor);
            //       fillHisto("track_iJet","TRK", samplename,tree_track_iJet->at(iTrk),NormFactor);// tracks matched with jets dR< 0.4
            //       fillHisto("track_track_Hemi_dR","TRK", samplename,tree_track_Hemi_dR->at(iTrk),NormFactor);// dR b/w tracks and jets
            //       fillHisto("track_track_Hemi_dRmax","TRK", samplename,tree_track_Hemi_dRmax->at(iTrk),NormFactor);
            //       fillHisto("track_lost","TRK", samplename,tree_track_lost->at(iTrk),NormFactor);
            //       fillHisto("track_MVAVal","TRK", samplename,tree_track_MVAval->at(iTrk),NormFactor);

            //       fillHisto("track_Track_firstHit","TRK", samplename,tree_track_firstHit->at(iTrk),NormFactor);
            //       fillHisto("track_Track_firstHit_x","TRK", samplename,tree_track_firstHit_x->at(iTrk),NormFactor);
            //       fillHisto("track_Track_firstHit_y","TRK", samplename,tree_track_firstHit_y->at(iTrk),NormFactor);
            //       fillHisto("track_Track_firstHit_z","TRK", samplename,tree_track_firstHit_z->at(iTrk),NormFactor);
            //       fillHisto2D("track_Track_firstHit_X_Y",  "TRK", samplename,tree_track_firstHit_x->at(iTrk),tree_track_firstHit_y->at(iTrk),NormFactor);
                  
            //       float firstHit_x =tree_track_firstHit_x->at(iTrk);
            //       float firstHit_y =tree_track_firstHit_y->at(iTrk);
            //       float firstHit_r=sqrt(firstHit_x*firstHit_x+firstHit_y*firstHit_y);
                  
            //       fillHisto("track_Track_firstHit_r","TRK", samplename,firstHit_r,NormFactor);
                                    
            //       float mva_ntrk10                  =  tree_track_ntrk10->at(iTrk);
            //       float mva_ntrk20                  =  tree_track_ntrk20->at(iTrk);
            //       float mva_ntrk30                  =  tree_track_ntrk30->at(iTrk);
            //       float mva_ntrk40                  =  tree_track_ntrk40->at(iTrk);
         ...
         Hemisphere1_pt = tree_Hemi_pt->at(0);
         Hemisphere2_pt = tree_Hemi_pt->at(1);
         ...
         for (unsigned int i = 0 ; i < tree_Hemi_pt->size(); i++)
            {
               fillHisto("Hemi_pt","",       samplename,  tree_Hemi_pt->at(i),NormFactor);
               fillHisto("Hemi_eta","",      samplename,  tree_Hemi_eta->at(i),NormFactor);
               fillHisto("Hemi_nJet","",     samplename,  tree_Hemi_njet->at(i),NormFactor);
               fillHisto("Hemi_nJetNoMu","",samplename,   tree_Hemi_njet_nomu->at(i),NormFactor);

               fillHisto("HemiMu_pt","",    samplename,  tree_HemiMu_pt->at(i),NormFactor);
               fillHisto("HemiMu_Mass","",  samplename,   tree_HemiMu_mass->at(i),NormFactor);
               fillHisto("Hemi_nTrks","",   samplename,  tree_Hemi_nTrks->at(i),NormFactor);
               fillHisto("Hemi_Mass","",    samplename,  tree_Hemi_mass->at(i),NormFactor);
            }

         for (unsigned int i = 0 ; i < tree_SecInt_pt->size(); i++)
            {
               if (tree_SecInt_selec->at(i))
                  {
                     fillHisto("SecInt_mass","Selec", samplename, tree_SecInt_mass->at(i), NormFactor);
                     fillHisto("SecInt_drSig","Selec", samplename, tree_SecInt_drSig->at(i), NormFactor);
                     fillHisto("SecInt_pt","Selec", samplename, tree_SecInt_pt->at(i), NormFactor);
                     fillHisto("SecInt_dzSig","Selec", samplename, tree_SecInt_dzSig->at(i), NormFactor);
                     fillHisto("SecInt_selec","Selec", samplename, tree_SecInt_selec->at(i), NormFactor);
                     fillHisto("SecInt_layer","Selec", samplename, tree_SecInt_layer->at(i), NormFactor);
                     fillHisto("SecInt_r","Selec", samplename, tree_SecInt_r->at(i), NormFactor);
                     fillHisto("SecInt_z","Selec", samplename, tree_SecInt_z->at(i), NormFactor);
                     if ( tree_SecInt_layer->at(i) )
                        {
                           fillHisto("SecInt_mass","TrackerMatched", samplename, tree_SecInt_mass->at(i), NormFactor);
                           fillHisto("SecInt_drSig","TrackerMatched", samplename, tree_SecInt_drSig->at(i), NormFactor);
                           fillHisto("SecInt_pt","TrackerMatched", samplename, tree_SecInt_pt->at(i), NormFactor);
                           fillHisto("SecInt_dzSig","TrackerMatched", samplename, tree_SecInt_dzSig->at(i), NormFactor);
                           fillHisto("SecInt_selec","TrackerMatched", samplename, tree_SecInt_selec->at(i), NormFactor);
                           fillHisto("SecInt_layer","TrackerMatched", samplename, tree_SecInt_layer->at(i), NormFactor);
                           fillHisto("SecInt_r","TrackerMatched", samplename, tree_SecInt_r->at(i), NormFactor);
                           fillHisto("SecInt_z","TrackerMatched", samplename, tree_SecInt_z->at(i), NormFactor);

                        }
                  }
            }
         // std::cout<<" here 5  "<<std::endl;
         for (unsigned int i = 0 ; i < tree_Hemi_Vtx_Mass->size(); i++)
            {
               fillHisto("Vtx_NChi2","",            samplename,  tree_Hemi_Vtx_NChi2->at(i),NormFactor);
               fillHisto("Vtx_nTrks","",           samplename,  tree_Hemi_Vtx_nTrks->at(i),NormFactor);
               fillHisto("Vtx_Mass","",         samplename,  tree_Hemi_Vtx_Mass->at(i),NormFactor);
               fillHisto("Vtx_Dist","",        samplename,   tree_Hemi_Vtx_dist->at(i),NormFactor);

               fillHisto("Vtx_Step","",            samplename,  tree_Hemi_Vtx_step->at(i),NormFactor);
                  fillHisto("Vtx_dR","",           samplename,  tree_Hemi_Vtx_dR->at(i),NormFactor);
            }
         // std::cout<<" here 6  "<<std::endl;
         for (unsigned int i = 0 ; i < tree_Hemi_SecVtx_Mass->size(); i++)
            {
               fillHisto("SecVtx_NChi2","",            samplename,  tree_Hemi_SecVtx_NChi2->at(i),NormFactor);
               fillHisto("SecVtx_nTrks","",           samplename,  tree_Hemi_SecVtx_nTrks->at(i),NormFactor);
               fillHisto("SecVtx_Mass","",         samplename,  tree_Hemi_SecVtx_Mass->at(i),NormFactor);
               fillHisto("SecVtx_Dist","",        samplename,   tree_Hemi_SecVtx_dist->at(i),NormFactor);

               fillHisto("SecVtx_Step","",         samplename,  tree_Hemi_SecVtx_step->at(i),NormFactor);
               fillHisto("SecVtx_dR","",        samplename,tree_Hemi_SecVtx_dR->at(i),NormFactor);
            }

         // std::cout<<" here 7  "<<std::endl;
         for (unsigned int i = 0 ; i < tree_Hemi_Vtx_BDT_nTrks->size() ; i++)
            {
               fillHisto("FinalVtx_nTrks","",           samplename,  tree_Hemi_Vtx_BDT_nTrks->at(i),NormFactor);
               fillHisto("FinalVtx_Step","",          samplename, tree_Hemi_Vtx_BDT_step->at(i),NormFactor);
               fillHisto("FinalVtx_Mass","",         samplename,  tree_Hemi_Vtx_BDT_HMass->at(i),NormFactor);
               fillHisto("FinalVtx_HMass","",        samplename,  tree_Hemi_Vtx_BDT_HMass->at(i),NormFactor);
            }
...
//----------------//
   ...
   addHisto("muon_pt", "NoSel", sample.Data(), 100, 0 ,300);
   addHisto("muon_eta", "NoSel", sample.Data(), 25,-2.4,2.4);
   addHisto("electron_pt", "NoSel", sample.Data(), 100, 0 ,300);
   addHisto("electron_eta", "NoSel", sample.Data(), 25,-2.4,2.4);
   ...
   addHisto("Hemi_Mass","",        sample.Data(),  50, 0 ,300);
...
// !! bgni
   ...
   addHisto("K0_pt","", sample.Data(), 50, 0 ,50);

   addHisto("L0_mass","", sample.Data(),202,1.06,1.18);
   addHisto("L0_pt","", sample.Data(), 500, 0 ,50);

   addHisto("Reco_K0_mass","", sample.Data(),202,0.42,0.58);
   addHisto("Reco_K0_pt","", sample.Data(), 50, 0 ,50);
   addHisto("Reco_L0_mass","", sample.Data(),202,1.06,1.18);
   addHisto("Reco_L0_pt","", sample.Data(), 50, 0 ,50);
   ...
   addHisto("SecInt_pt","Selec", sample.Data(),100, 0 ,100);
   ...
   addHisto("SecInt_r","Selec", sample.Data(), 200,0,20);
   ...
   addHisto("SecInt_pt","TrackerMatched", sample.Data(),100, 0 ,100);
   ...
   addHisto("SecInt_r","TrackerMatched", sample.Data(), 200,0,20);
   addHisto("SecInt_z","TrackerMatched", sample.Data(), 800,-200,200);
// !! end 
